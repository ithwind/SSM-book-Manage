...
<div class="wrapper">
        ...
        <div class="sidebar os-padding">
            ...
            <nav class="mt-2">
                <ul class="nav nav-pills nav-sidebar flex-column" data-widget="treeview" role="menu" data-accordtion="false">
                    ...
                    <li class="nav-item">
                        <a href="${pageContext.request.contextPath}/searchBorrow" class="nav-link active">
                            <i class="nav-icon fas fa-tachometer-alt"></i>
                            <p>
                                当前借阅
                                <i class="right fas fa-angle-left"></i>
    ...
    <!-- 导航侧栏 /-->
    <div class="content-wrapper" style="min-height: 955px;" href="${pageContext.request.contextPath}/searchBorrow">
        <!-- 主体内容 -->
                    ...
                    <div class="card-body p-0">
                        <div class="table-responsive">
                            <table class="table m-0">
                                ...
                                <tbody>
                                <c:forEach items="${borrow_book.rows}" var="record">
                                    <tr>
                                        ...
                                        <td class="text-center">
                                            <c:if test="${record.book_status == 1}">
                                                <button type="button" class="btn bg-olive btn-xs" onclick="returnBook(${record.book_id})">归还图书</button>
                                            </c:if>
                                            <c:if test="${record.book_status == 2}">
                                                <button type="button" class="btn bg-olive btn-xs" onclick="returnBook(${record.book_id})" disabled>等待确认</button>
                                            </c:if>
                                        </td>
                                    </tr>
                                </c:forEach>
...
<script type="text/javascript">
    ...
    function returnBook(bookId){
        const flag = confirm("确定归还图书");
        if(flag){
            const url = getProjectPath() + "/returnBook?id=" + bookId;
            $.post(url, function (response){
                alert(response.message);
                if(response.success === true){
                    window.location.href = getProjectPath() + "/searchBorrow";
                }
            })
        }
